...
def autodiscover_templates():
    ...
    override_dir = getattr(settings, 'CMSPLUGIN_COLUMNS_TEMPLATES', None)
    if override_dir:
        return sorted_templates(override_dir)

    templates = []
#    templates = [
#        ('cmsplugin_columns/columns.html', 'columns.html'),
#    ]
    ...
    for dir in dirs_to_scan:
        found = glob.glob(os.path.join(dir, 'djangocms_columns/*.html'))
        for file in found:
